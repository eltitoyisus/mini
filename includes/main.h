...
/*   Updated: 2025/06/22 14:43:45 by jramos-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// GAME
void				init_game(t_snake *snake, t_position *fruit,
						t_state *state);
...
void				case_without_space(char *sp_input, char **env,
						t_parse *parse);
...
void				handle_command_parts(char **cmd_parts, t_parse **parse,
						char **env);
void				process_command_with_spaces(char *cmd_segment,
...
void				check_command_features(t_sh *sh, int *has_pipe,
						int *has_redirs, int *has_heredoc);
int					create_default_cat(t_sh *sh, int has_redirs,
						int stdin_backup, int *needs_cleanup);
...
int					setup_command_redirections(t_sh *sh, t_reds **redirs,
						char ***clean_args);
int					get_command_path(char **envp, char **clean_args,
						t_reds *redirs, char **path);
void				setup_redirection_fds(t_reds *redirs, int *stdin_fd,
						int *stdout_fd);
void				handle_fork_error(char *path, t_reds *redirs,
...
void				cleanup_resources(char *path, t_reds *redirs,
...
// ECHO
int					check_quotes_type(char *input, int len,
						int *is_single_quoted, int *is_double_quoted);
...
void				copy_char_to_result(char *input, char *result, int *i,
						int *j);
...
void				cleanup_parent_process(t_reds *redirs, pid_t pid,
						int *status);
...
void				setup_redir_fds(t_reds *redirs, int *stdin_fd,
						int *stdout_fd);
...
int					heredoc_fork_and_wait(int fd, char *delimiter,
						char *filename, t_heredoc_state *state);
...
t_pipe_cmd_args		init_pipe_cmd_args(t_pipe_handle *ph, t_sh *sh,
						char **envp);
